...
const WORK_ENTRIES: WorkEntry[] = [
  ...
  {
    date: "Digital drawing",
    title: "Ideas, made visible",
    bio: "Using Procreate taught me to turn rough ideas into visual form quickly, then refine them through iteration.",
    orbitRadius: 0.74,
    speed: 0.25,
    phase: -0.25,
    size: 108,
    image: "assets/orbit-icons/procreate.png",
    link: "projects/",
  },
];
...
function initWorkOrbit(container: HTMLElement): void {
  ...
  const bubbleLink = bubble.querySelector(".work-orbit__bubble-link") as HTMLAnchorElement;
  ...
  const planetEls: HTMLAnchorElement[] = WORK_ENTRIES.map((entry, i) => {
  ...
  function drawPlanets(cx: number, cy: number, maxR: number): void {
    ...
    function updatePosition(i: number): void {
    ...
    for (let pass = 0; pass < 6; pass++) {
      for (let i = 0; i < layouts.length; i++) {
        for (let j = i + 1; j < layouts.length; j++) {
          const a = layouts[i];
          const b = layouts[j];
          if (a.x + a.size / 2 < 0 && b.x + b.size / 2 < 0) continue;
