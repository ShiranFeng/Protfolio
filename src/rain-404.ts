...
function init404Rain(canvas: HTMLCanvasElement): void {
  ...
  function makeDrop(randomY = true): RainDrop {
    return {
  ...
  function resetDrops(): void {
    drops.length = 0;
    for (let i = 0; i < RAIN_COUNT; i++) drops.push(makeDrop());
  }
  ...
  function deflectPoint(x: number, y: number): { x: number; y: number; masked: boolean } {
    if (!pointer.active) return { x, y, masked: false };
  ...
  function drawDrop(drop: RainDrop): void {
    const samples = 44;
    let drawing = false;
    ctx!.beginPath();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (canvas instanceof HTMLCanvasElement) init404Rain(canvas);
});
